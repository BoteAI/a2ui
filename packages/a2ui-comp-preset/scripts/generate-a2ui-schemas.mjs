...
export async function generateA2uiSchemas(root, userConfig) {
  ...
  const outdir = schemaConfig.outdir ?? 'src/schemas';
  const registryInput = schemaConfig.registry ?? 'scripts/a2ui-preset-schema-registry.ts';
  ...
  if (!Array.isArray(docs) || docs.length === 0) {
    throw new Error('[generate-a2ui-schema] schemaRegistry 为空，请检查 scripts/a2ui-preset-schema-registry.ts');
  }
